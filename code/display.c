...
#include "allheaderfile.h"
    ...
    Image* load_bmp_image(const char *filename) {
        ...
        if (info[0] != 'B' || info[1] != 'M') {
            IupMessage("Error", "Not a valid BMP File.");  
            fclose(f); 
            return NULL;
        }
        ...
        if (bpp != 24 || compression != 0) {
            IupMessage("Error", "Only 24-bit uncompressed BMP is supported.");
            fclose(f);
            return NULL;
        }
        ...
        for (int y = height - 1; y >= 0; y--) {
            ...
            if (row_padded > width * 3) fseek(f, row_padded - width * 3, SEEK_CUR);
        }
        fclose(f);
    ...
    void bmp_open() {
        ...
        if (IupGetInt(file_dlg, "STATUS") != -1) {
            char *filename = IupGetAttribute(file_dlg, "VALUE");
                    if (filename) {
                Image *loaded = load_bmp_image(filename);
                if (loaded) {
                    if (current_image) { free(current_image->data); free(current_image); }
                    current_image = loaded;
                    update_display();
                } else {
                    IupMessage("Error", "Failed to open image. Please select a valid 24-bit uncompressed BMP file.");
                }
            }
        }
        IupDestroy(file_dlg);
    ...
    int save_bmp_image(const char *filename) {
        ...
        for (int y = height - 1; y >= 0; y--) {
            ...
            if (pad) fwrite(pad_bytes, 1, pad, f);
        }
        fclose(f);
